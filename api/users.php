                        ...
                        if ($result) {
                            ...
                            if ($result->num_rows > 0) {
                                while ($row = $result->fetch_assoc()) {
                                    ...
                                    $rolename = '';
                                    if ($roleid) {
                                        $rolename = getProperties('roles', 'name', 'id', $roleid);
                                    }
                                    //count tickets
                                    $associatedTickets = countRecords('tickets', 'raised_by_user_id', $id);

                                    $users[] = [
                                        'id' => $row['id'],
                                        'full_name' => $row['full_name'],
                                        'ad_account_no' => $row['ad_account_no'],
                                        'email' => $row['email'],
                            ...
                            $httpResponseCode = 200;
                            $response_array = $users;

                            logAction($currentUserAdAccount, $currentUserId, $token, $httpResponseCode, "Fetching users $countUsers users records found.");
                        } else {
                            $httpResponseCode = 500;
                            $error = "Query failed";
                            $message = "Query failed: " . $conn->error;
...
if ($httpResponseCode != '200') {
    ...
    logAction($currentUserAdAccount ?? null, $currentUserId ?? null, $token ?? null, $httpResponseCode, "Fetching users failed: Error: $error. Message $message");
}
