        ...
        if (substr($headers['Authorization'], 0, 7) !== 'Bearer ') {
            ...
            if (!isTokenValid($token)) {
                ...
                if (!refreshToken($token)) {
                    ...
                    //process Actual API
                    try {
                        $sql = "SELECT id, region_name as name, region_code as code FROM regions ORDER BY region_name ASC";
                        $result = $conn->query($sql);
                        $regions = []; // Initialize an array to hold all regions data
                        if ($result) {
                            $countRegions = $result->num_rows;
                            ...
                            $httpResponseCode = 200;
                            $response_array = $regions;

                            logAction($currentUserAdAccount, $currentUserId, $token, $httpResponseCode, "Fetching regions. $countRegions records found.");
                        } else {
                            $httpResponseCode = 500;
                            $error = "Query failed";
                            $message = "Query failed: " . $conn->error;
...
if ($httpResponseCode != '200') {
    ...
    logAction($currentUserAdAccount ?? null, $currentUserId ?? null, $token ?? null, $httpResponseCode, "Fetching regions failed: Error: $error. Message $message");
}
