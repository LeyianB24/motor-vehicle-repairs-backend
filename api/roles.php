        ...
        if (substr($headers['Authorization'], 0, 7) !== 'Bearer ') {
            ...
            if (!isTokenValid($token)) {
                ...
                if (!refreshToken($token)) {
                    ...
                    //process Actual API
                    try {
                        $sql = "SELECT * FROM roles WHERE status='1' ORDER BY name ASC";
                        ...
                        if ($result) {
                            ...
                            $httpResponseCode = 200;
                            $response_array = $roles;

                            logAction($currentUserAdAccount, $currentUserId, $token, $httpResponseCode, "Fetching roles. $countUsers records found.");
                        } else {
                            $httpResponseCode = 500;
                            $error = "Query failed";
                            $message = "Query failed: " . $conn->error;
